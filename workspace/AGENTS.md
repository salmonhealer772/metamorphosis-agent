...
Mem0 handles all memory automatically — auto-capture and auto-recall.
No manual `memory_search` or `ov.py` calls needed.
On startup, check `openclaw mem0 status` — if it errors, warn the user.
