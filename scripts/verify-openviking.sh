...
PASS=0
FAIL=0
OV_DIR="$HOME/.openclaw/workspace/.openviking"
OV_PY="$HOME/.openclaw/workspace/ov.py"
...
echo -e "\n[3] CLI Sanity"
check "ov.py status works"      "cd \"$(dirname \"$OV_PY\")\" && python3 '$OV_PY' status 2>&1 | grep -q 'Semantic search: OK'"
check "ov.py ls returns items"  "cd \"$(dirname \"$OV_PY\")\" && python3 '$OV_PY' ls 2>&1 | grep -qE '📁|📄'"
check "ov.py find returns hits" "cd \"$(dirname \"$OV_PY\")\" && python3 '$OV_PY' find 'openviking memory' 2>&1 | grep -q 'Found'"

# ── Layer 4: E2E Store → Find → Delete on real workspace ──
echo -e "\n[4] End-to-End (Real Workspace)"
MARKER="OV_VERIFY_$(date +%s)"
if OV_VERIFY_MARKER="$MARKER" python3 "$HOME/.openclaw/workspace/verify-e2e.py" 2>/dev/null; then
