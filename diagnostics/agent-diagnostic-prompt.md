...
- `openclaw mem0 search "diagnostic" --user-id <agent-name>` — should return results

### 2. Auto-Capture
- Mem0 auto-captures every message — vector DB should have entries
...
### 4. Ollama / embeddings
- `curl -s http://127.0.0.1:11434/api/tags | python3 -c "import sys,json; print([m['name'] for m in json.load(sys.stdin)['models']])"` — should include nomic-embed-text

### 5. File structure
- `ls -d .mem0/vector_store.db 2>/dev/null && echo "EXISTS"` — should exist
